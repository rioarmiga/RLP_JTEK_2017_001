...
class ThemexWoo {
		...
		if(self::isActive()) {	
			...
			if(!empty($object)) {
				...
				foreach($order['products'] as &$product) {
					...
					$product['meta']=new WC_Order_Item_Meta($product);
				}
			...
			if(isset($data['add'])) {
				...
				if(!empty($products)) {
					...
					foreach($cart[$ID] as $product) {
						self::$woocommerce->cart->add_to_cart($product['product_id'], $product['quantity'], $product['variation_id'], $product['variation']);
					}
	...
	public static function updateShipping($rates, $package) {
		...
		$shop=ThemexUser::getShop($product['data']->post->post_author);
